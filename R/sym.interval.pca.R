...
#' @export
#'
sym.interval.pca <- function(sym.data, method = c("classic", "tops",
                                                    "centers", "principal.curves", "optimized.distance", "optimized.variance")) {
  ...
  if (method == "classic") {
    ...
    if (sym.data$sym.var.types[1] == "$C")
      res <- PCA(sym.data$data, scale.unit = TRUE, ncp = sym.data$M,
                 graph = FALSE) else if (sym.data$sym.var.types[1] == "$I") {
                   nn <- sym.data$N
                   mm <- sym.data$M
                   ...
                   for (i in 1:nn) for (j in 1:mm) centers[i, j] <- (sym.var(sym.data,
                                                                             j)$var.data.vector[i, 1] + sym.var(sym.data, j)$var.data.vector[i,
                                                                                                                                             2])/2
                   res <- FactoMineR::PCA(centers, scale.unit = TRUE, ncp = sym.data$M,
                                          graph = FALSE)
                 }
    return(res)
  }
  if (method == "centers") {
    ...
    for (i in 1:nn) for (j in 1:mm) centers[i, j] <- (sym.var(sym.data,
                                                              j)$var.data.vector[i, 1] + sym.var(sym.data, j)$var.data.vector[i,
                                                                                                                              2])/2
    # Standarized
    for (i in 1:nn) for (j in 1:mm) centers.stan[i, j] <- (centers[i,
                                                                   j] - mean(centers[, j]))/(sd(centers[, j]) * sqrt((nn -
                                                                                                                        1)/nn))
    # Min-Max
    for (i in 1:nn) {
      for (j in 1:mm) {
        min.stan[i, j] <- (sym.var(sym.data, j)$var.data.vector[i,
                                                                1] - mean(centers[, j]))/(sd(centers[, j]) * sqrt((nn -
                                                                                                                     1)/nn))
        max.stan[i, j] <- (sym.var(sym.data, j)$var.data.vector[i,
                                                                2] - mean(centers[, j]))/(sd(centers[, j]) * sqrt((nn -
                                                                                                                     1)/nn))
      }
    }
        ...
        for (k in 1:mm) {
          if (svd$vectors[k, j] < 0)
            smin <- smin + max.stan[i, k] * svd$vectors[k,
                                                        j] else smin <- smin + min.stan[i, k] * svd$vectors[k,
                                                                                                            j]
                                                        if (svd$vectors[k, j] < 0)
                                                          smax <- smax + min.stan[i, k] * svd$vectors[k,
                                                                                                      j] else smax <- smax + max.stan[i, k] * svd$vectors[k,
                                                                                                                                                          j]
        }
    ...
    for (i in 1:mm) {
      ...
      for (j in 1:mm) {
        ...
        for (k in 1:nn) {
          if (svdV[k, j] < 0)
            smin <- smin + (1/sqrt(nn)) * max.stan[k, i] *
              svdV[k, j] else smin <- smin + (1/sqrt(nn)) * min.stan[k, i] *
                  svdV[k, j]
              if (svdV[k, j] < 0)
                smax <- smax + (1/sqrt(nn)) * min.stan[k, i] *
                  svdV[k, j] else smax <- smax + (1/sqrt(nn)) * max.stan[k, i] *
                      svdV[k, j]
        }
  ...
  if (method == "principal.curves") {
    res <- sym.interval.pc(sym.data, "vertex", 150, FALSE, FALSE,
                           TRUE)
    return(res)
  }
