...
type.set <- function(){
  set <- function(x){
    ...
    out.df <- cbind(names(split.df),"$S",length(levels(new.factor)),props.df)
    colnames(out.df) <- c("concept","$S",var.name,levels(new.factor))
    out.df
  }
...
create.meta.data <- function(x = NULL, col.types = NULL){
  ...
  suppressWarnings(meta.data <- Reduce(function(dtf1, dtf2) merge(dtf1, dtf2, by = "concept", suffixes = c("",""), all = TRUE),
                                       meta.data))
