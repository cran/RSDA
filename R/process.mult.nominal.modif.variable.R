...
process.mult.nominal.modif.variable <- function(labels, number.of.rows, parsed.xml, 
    ...
    if (labels) 
        categories <- xpathSApply(parsed.xml, paste0("/assofile/variables/stvar[", 
            variable.index, "]/mult_nominal_Modif/nominal-desc/list-nom/label"), xmlValue) else categories <- xpathSApply(parsed.xml, paste0("/assofile/variables/stvar[", 
        variable.index, "]/mult_nominal_Modif/nominal-desc/list-nom/name"), xmlValue)
