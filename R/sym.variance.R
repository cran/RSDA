...
sym.variance <- function(sym.var, method = c("centers", "interval", "billard", "modal"), 
    ...
    if (method == "centers") {
        ...
        if (sym.var$var.type == "$I") 
            return(var(sym.var$var.data.vector[, 1] + sym.var$var.data.vector[, 2])/2) else stop("Impossible to compute the variance for this type of variable with this method")
    }
    if (method == "interval") {
        if (sym.var$var.type == "$I") 
    ...
    if (method == "billard") {
        if (sym.var$var.type == "$I") 
            return((1/(3 * sym.var$N)) * sum(sym.var$var.data.vector[, 1]^2 + (sym.var$var.data.vector[, 
                1] * sym.var$var.data.vector[, 2]) + sym.var$var.data.vector[, 2]^2) - 
