...
SDS.to.RSDA <- function(file.path, labels = FALSE) {
    ...
    regex.preprocess <- function(SdsData) {
        ...
        SdsData <- sub(pattern = "^(?:NA|NU)$", replacement = "null", x = SdsData, 
            perl = TRUE)  # We replace NA and NU with nulls.
        SdsData <- sub(pattern = "^\\.(\\d+)$", replacement = "0.\\1", x = SdsData, 
            perl = TRUE)  # We fill numbers that do not have the integer part (for example we replace .12 with 0.12).
    ...
    process.mult.nominal.modif.variable <- function(labels, number.of.rows, data, variable.index, 
        ...
        for (i in 1:length(categories)) {
            aux[[i + 2]] <- sapply(X = data$SODAS$RECTANGLE_MATRIX, FUN = function(dat.ind) {
                ...
                if (is.null(val)) 
                  return(NA) else return(ifelse(test = is.na(val), yes = 0, no = round(val, 3)))
            })
        }
    ...
    for (i in 1:length(variables.types)) {
        ...
        switch(variables.types[[i]], inter_continue = {
            result <- cbind(result, process.inter.cont.variable(number.of.rows, data, 
                i, variables.names[[i]]))
        }, inter_cont = {
            result <- cbind(result, process.inter.cont.variable(number.of.rows, data, 
                i, variables.names[[i]]))
        }, continue = {
            result <- cbind(result, process.continue.variable(number.of.rows, data, 
                i, variables.names[[i]]))
        }, continu = {
            result <- cbind(result, process.continue.variable(number.of.rows, data, 
                i, variables.names[[i]]))
            ...
            result <- cbind(result, process.mult.nominal.modif.variable(labels, number.of.rows, 
                data, i, variables.names[[i]]))
