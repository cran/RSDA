...
#'
#'
sym.cov <- function(sym.var.x, sym.var.y, method = c("centers", "interval", "billard", 
    "modal"), na.rm = FALSE, ...) {
        ...
        if ((sym.var.x$var.type == "$I") && (sym.var.y$var.type == "$I")) {
            ...
            for (i in 1:(sym.var.x$N)) {
                ss <- ss + Gj(sym.var.x$var.data.vector[i, 1], sym.var.x$var.data.vector[i, 
                  2], vmean.x) * Gj(sym.var.y$var.data.vector[i, 1], sym.var.y$var.data.vector[i, 
                  2], vmean.y) * sqrt(Qj(sym.var.x$var.data.vector[i, 1], sym.var.x$var.data.vector[i, 
                  2], vmean.x) * Qj(sym.var.y$var.data.vector[i, 1], sym.var.y$var.data.vector[i, 
                  2], vmean.y))
            }
