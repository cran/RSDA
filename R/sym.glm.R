...
sym.glm <- function(sym.data, response = 1, method = c("cm", "crm"), alpha = 1, nfolds = 10, 
    ...
    if (method == "cm") {
        ...
        model <- cv.glmnet(centers[, -response], centers[, response], nfolds = nfolds, 
    ...
    if (method == "crm") {
        ...
        for (i in 1:nn) for (j in 1:mm) centers[i, j] <- (sym.var(sym.data, j)$var.data.vector[i, 
            1] + sym.var(sym.data, j)$var.data.vector[i, 2])/2
        modelc <- cv.glmnet(centers[, -response], centers[, response], nfolds = nfolds, 
            grouped = grouped, alpha = alpha)
        # Range Model
        ...
        modelr <- cv.glmnet(range[, -response], range[, response], nfolds = nfolds, 
            grouped = grouped, alpha = alpha)
